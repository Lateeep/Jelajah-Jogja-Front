...
const MapPage = {
  ...
  async afterRender() {
    ...
    // Buat layer group biasa sebagai pengganti MarkerCluster
    ...
    document.querySelector(".btn-destinasi").addEventListener("click", () => {
      markersGroup.clearLayers(); // Bersihkan marker yang ada
      ...
      if (titikAkhir !== "") {
        ...
        if (area.length !== 0) {
          area.forEach((data) => {
            ...
            markersGroup.addLayer(markerWisata);
          });
          area = [];
        }
      }
    ...
    // Cek apakah ada koordinat tracking dari halaman hotel
    const trackingPoints = JSON.parse(localStorage.getItem('trackingPoints'));
    if (trackingPoints) {
      const startPoint = L.latLng(parseFloat(trackingPoints.start.lat), parseFloat(trackingPoints.start.lng));
      const endPoint = L.latLng(parseFloat(trackingPoints.end.lat), parseFloat(trackingPoints.end.lng));
      
      // Buat routing langsung menggunakan L.Routing.control
      const routing = L.Routing.control({
        ...
        createMarker: function(i, waypoint) {
          const marker = L.marker(waypoint.latLng, {
            draggable: false,
            icon: i === 0 ? iconWisata : iconBecak
          });
          return marker;
        }
      }).addTo(map);
      
      // Hapus data tracking dari localStorage
      localStorage.removeItem('trackingPoints');
      
      // Update panduan
      panduanContainer.innerHTML = `
        <div class="panduan_item" data-aos="fade-up" data-aos-delay="50" 
          data-aos-duration="1500" data-aos-easing="ease-in-out">
          <h1>Rute dari Wisata ke Hotel</h1>
          <p>Menampilkan rute perjalanan dari lokasi wisata ke hotel yang dipilih.</p>
        </div>
      `;
